...
class MultiViewControlDiT(MinimalV4LVGControlVaceDiT):
    ...
    def prepare_embedded_sequence(
        ...
        cp_size = 1 #len(get_process_group_ranks(process_group))
    ...
    #     # 原逻辑: n_cameras = (x_B_C_T_H_W.shape[2] * cp_size) // self.state_t
    #     # 单卡时 cp_size = 1
    ...
    #     # =====处理视角嵌入（串行分块）=====
    #     if self.concat_view_embedding:
    #         # 重排为视角维度: [B, C, (V*T), H, W] -> [B, C, V, T, H, W]
    #         x_B_C_V_T_H_W = rearrange(x_B_C_T_H_W, "B C (V T) H W -> B C V T H W", V=n_cameras)
            
    #         # 串行处理每个视角（或分块处理）
    #         embedded_chunks = []
    #         chunk_size = self.get_optimal_chunk_size(n_cameras)  # 可配置的块大小
            
    #         for start_v in range(0, n_cameras, chunk_size):
    #             end_v = min(start_v + chunk_size, n_cameras)
    #             chunk_views = x_B_C_V_T_H_W[:, :, start_v:end_v]  # [B, C, chunk_V, T, H, W]
                
    #             # 提取当前块的视角索引
    #             if view_indices_B_T is None:
    #                 chunk_view_indices = torch.arange(start_v, end_v).clamp(
    #                     max=self.n_cameras_emb - 1
    #                 ).to(x_B_C_T_H_W.device)
    #                 chunk_view_embedding = self.view_embeddings(chunk_view_indices)  # [chunk_V, D]
    #                 chunk_view_embedding = rearrange(chunk_view_embedding, "V D -> D V")
    #                 chunk_view_embedding = chunk_view_embedding.unsqueeze(0).unsqueeze(3).unsqueeze(4).unsqueeze(5)
    #             else:
    #                 # 处理动态视角索引（需要按块切分）
    #                 chunk_view_indices_B_T = view_indices_B_T[:, start_v*self.state_t:end_v*self.state_t]
    #                 chunk_view_indices_B_T = chunk_view_indices_B_T.clamp(max=self.n_cameras_emb - 1)
    #                 chunk_view_indices_B_T = chunk_view_indices_B_T.to(x_B_C_T_H_W.device).long()
    #                 chunk_view_embedding = self.view_embeddings(chunk_view_indices_B_T)  # [B, (chunk_V*T), D]
    #                 chunk_view_embedding = rearrange(chunk_view_embedding, "B (V T) D -> B D V T", V=end_v-start_v)
    #                 chunk_view_embedding = chunk_view_embedding.unsqueeze(-1).unsqueeze(-1)
                
    #             # 扩展视角嵌入到匹配数据形状
    #             chunk_view_embedding = chunk_view_embedding.expand(
    #                 chunk_views.shape[0],
    #                 chunk_view_embedding.shape[1],
    #                 chunk_view_embedding.shape[2],
    #                 chunk_views.shape[3],
    #                 chunk_views.shape[4],
    #                 chunk_views.shape[5],
    #             )
    ...
    #             # 拼接视角嵌入
    #             chunk_with_embedding = torch.cat([chunk_views, chunk_view_embedding], dim=1)
    #             embedded_chunks.append(chunk_with_embedding)
            
    #         # 合并所有块
    #         x_B_C_V_T_H_W = torch.cat(embedded_chunks, dim=2)
    #         x_B_C_T_H_W = rearrange(x_B_C_V_T_H_W, "B C V T H W -> B C (V T) H W", V=n_cameras)
