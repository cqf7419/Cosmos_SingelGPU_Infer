...
class MultiviewControlVideo2WorldModelRectifiedFlow(ControlVideo2WorldModelRectifiedFlow):
    ...
    def encode(self, state: torch.Tensor) -> torch.Tensor:
        n_views = state.shape[2] // self.tokenizer.get_pixel_num_frames(self.state_t)
        state = rearrange(state, "B C (V T) H W -> V B C T H W", V=n_views)
        # 串行处理
        encoded_views = []
        chunk_size = 1
        for i in range(0, n_views, chunk_size):
            # [chunk, B, C, T, H, W] -> [chunk*B, C, T, H, W]
            chunk_views = state[i:i+chunk_size].contiguous()
            chunk_views = rearrange(chunk_views, "V B C T H W -> (B V) C T H W")
            encoded_chunk = super().encode(chunk_views)  # [B, C, T, H, W]
            encoded_chunk = rearrange(encoded_chunk, "(B V) C T H W -> V B C T H W", V=chunk_views.shape[0])
            encoded_views.extend([encoded_chunk[v] for v in range(encoded_chunk.shape[0])])
        
        # [V, B, C, T, H, W] -> [B, C, (V T), H, W]
        encoded_state = torch.cat(encoded_views, dim=2)
        return encoded_state
    ...
    def decode(self, latent: torch.Tensor) -> torch.Tensor:
        n_views = latent.shape[2] // self.state_t
        latent = rearrange(latent, "B C (V T) H W -> V B C T H W", V=n_views)
        
        decoded_views = []
        chunk_size = 1
        for i in range(0, n_views, chunk_size):
            chunk_latent = latent[i:i+chunk_size].contiguous()
            chunk_latent = rearrange(chunk_latent, "V B C T H W -> (B V) C T H W")
            decoded_chunk = super().decode(chunk_latent)  # [B, C, T, H, W]
            decoded_chunk = rearrange(decoded_chunk, "(B V) C T H W -> V B C T H W", V=chunk_latent.shape[0])
            decoded_views.extend([decoded_chunk[v] for v in range(decoded_chunk.shape[0])])
        
        decoded_state = torch.cat(decoded_views, dim=2)
        return decoded_state
