...
class _CheckpointLocal(_CheckpointUri):
    """Config for checkpoint from local path."""
    ...
    def _download(self) -> str:
        ...
        if not os.path.exists(self.path_str):
            raise FileNotFoundError(f"Local checkpoint not found: {self.path_str}")
...
class CheckpointConfig(pydantic.BaseModel):
    ...
    local: _CheckpointLocal | None = None 
    """Config for checkpoint from local path."""
    ...
    @cached_property
    def path(self) -> str:
        """Return local path, S3 URI or local path."""
        # 1. 优先使用本地路径
        if self.local is not None:
            log.info(f"Using local checkpoint for {self.name}({self.uuid})")
            return self.local.path
        # 2、应该是nvidia内部使用的bucket
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/Cosmos-Drive-Dreams/cosmos-transfer1/checkpoints/google-t5/t5-11b"
        ),
        # hf=CheckpointDirHf(
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/Cosmos-Drive-Dreams/cosmos-transfer1/checkpoints/meta-llama/Llama-Guard-3-8B"
        ),
        # hf=CheckpointDirHf(
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/Cosmos-Drive-Dreams/cosmos-transfer1/checkpoints/nvidia/Cosmos-Guardrail1"
        ),
        # hf=CheckpointDirHf(
...
_register_checkpoint(
    CheckpointConfig(
        ...
        name="Wan2.1/vae",
        local=_CheckpointLocal(
            path_str="/mnt_gx/jiarun/VACE/models/Wan2.1-VACE-1.3B/Wan2.1_VAE.pth"
        ),
        s3=CheckpointFileS3(
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/Cosmos-reason1/cosmos-reason1"
        ),
        s3=CheckpointDirS3(
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/Cosmos-predict/cosmos-predict2.5/auto/multiview/6b9d7548-33bb-4517-b5e8-60caf47edba7_ema_bf16.pt"
        ),
        metadata={
            "resolution": "720p",
            "fps": 30,
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/cosmos-transfer2.5-main/cosmos_transfer2/checkpoints/general/edge"
        ),
        metadata={
            "resolution": "720p",
            "fps": 16,
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/cosmos-transfer2.5-main/cosmos_transfer2/checkpoints/general/seg"
        ),
        metadata={
            "resolution": "720p",
            "fps": 16,
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/cosmos-transfer2.5-main/cosmos_transfer2/checkpoints/general/blur"
        ),
        metadata={
            "resolution": "720p",
            "fps": 16,
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/cosmos-transfer2.5-main/cosmos_transfer2/checkpoints/general/depth"
        ),
        metadata={
            "resolution": "720p",
            "fps": 16,
...
_register_checkpoint(
    CheckpointConfig(
        ...
        local=_CheckpointLocal(
            path_str="/data1/cqf/cosmos-transfer2.5-main/cosmos_transfer2/checkpoints/auto/multiview/b5ab002d-a120-4fbf-a7f9-04af8615710b_ema_bf16.pt"
        ),
        metadata={
            "resolution": "720p",
            "fps": 16,
...
def get_checkpoint_path(checkpoint_uri: str) -> str:
    ...
    checkpoint_uri = checkpoint_uri.rstrip("/") # 移除末尾的 / 
